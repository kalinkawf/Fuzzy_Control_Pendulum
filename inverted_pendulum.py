...
class InvertedPendulum(QtWidgets.QWidget):
    ...
    def single_loop_run(self):
        for i in range(self.frameskip+1):
            ...
            print(f"Polozenie wozka : {self.x}, predkosc wozka : {self.theta}, polozenie wahadla : {self.dx}, predkosc wahadla : {self.dtheta}")
            # if (min_theta > self.theta):
            #     min_theta = self.theta
            # if (max_theta < self.theta):
            #     max_theta = self.theta
            # if (min_dtheta > self.dtheta):
            #     min_dtheta = self.dtheta
            # if (max_dtheta < self.dtheta):
            #     max_dtheta = self.dtheta
            # print(f"Min theta : {min_theta}, max_theta : {max_theta}, min dtheta : {min_dtheta}, max dtheta: {max_dtheta}")
    ...
    def defuzzyfy(self, fuzzy_output):
        return fuzzy_output

    def helper_conjunction(self, a, b):
        return min(a, b)

    def helper_alternative(self, a, b):
        return max(a, b)

    def helper_negative(self, a):
        return 1 - a

    def helper_normalize(self, value, min_val, max_val):
